...
const HouseholdSetup = ({ onSetup, onClose, isOpen, currentUser, onReturnToHousehold, hasExistingHousehold }) => {
  const [formData, setFormData] = useState({
    email: '',
    action: 'create' // 'create', 'join', 'return'
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ email: '', action: 'create' })
      onClose()
    } catch (err) {
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {formData.action !== 'return' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {formData.action === 'create' ? 'Email del propietario' : 'Email del propietario del hogar'}
                </label>
                <input
                  type="email"
                  name="email"
                  ...
                  onChange={handleChange}
                  placeholder={formData.action === 'create' ? 'tu@email.com' : 'propietario@email.com'}
