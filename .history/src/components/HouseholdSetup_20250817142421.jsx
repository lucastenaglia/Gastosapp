...
const HouseholdSetup = ({ onSetup, onClose, isOpen, currentUser, onReturnToHousehold, hasExistingHousehold, onInviteUser }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email.trim() && formData.action !== 'create') {
      setError('Por favor ingresa el email requerido')
      return
    }
    ...
    try {
      // Si es crear hogar, usar el email del usuario actual
      if (formData.action === 'create') {
        await onSetup(currentUser.email, formData.action, formData.invitedEmails)
      } else {
      ...
      >
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {formData.action !== 'return' && formData.action !== 'create' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {formData.action === 'join' ? 'Email del propietario del hogar' : 'Email del usuario a invitar'}
                </label>
                <input
                  type="email"
                  name="email"
                  ...
                  onChange={handleChange}
                  placeholder={formData.action === 'join' ? 'propietario@email.com' : 'usuario@email.com'}
            ...
            {/* Información del propietario para crear hogar */}
            {formData.action === 'create' && (
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Propietario del hogar:</strong> {currentUser?.email}
                </p>
                <p className="text-xs text-blue-600 mt-1">
                  Tu email será automáticamente el propietario del nuevo hogar.
                </p>
              </div>
            )}

            {/* Sección para agregar invitados al crear hogar */}
            {formData.action === 'create' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Invitar usuarios al hogar (opcional)
                </label>
                ...
                {formData.invitedEmails.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs text-gray-600">Usuarios invitados:</p>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Los usuarios invitados serán agregados automáticamente al hogar y podrán ver todos los gastos compartidos.
                </p>
              </div>
            )}
