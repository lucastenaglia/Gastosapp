...
const ExpenseSummary = ({ expenses, isPersonal = false, householdMembers = [], onFilterPerson = null, categories = [] }) => {
  ...
  // Calcular gastos de café
  const coffeeExpenses = expenses.filter(expense => 
    expense.category === 'cafe' || 
    expense.description?.toLowerCase().includes('café') ||
    expense.description?.toLowerCase().includes('cafe') ||
    expense.description?.toLowerCase().includes('coffee')
  )
  const coffeeTotal = coffeeExpenses.reduce((sum, expense) => sum + expense.amount, 0)
        ...
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow border hover:shadow-lg hover:scale-105 transition-all duration-200 cursor-pointer col-span-2 sm:col-span-1">
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-lg sm:text-xl font-bold text-gray-900">
                  {formatCurrency(coffeeTotal)}
                </p>
                <p className="text-sm text-gray-600">
                  {coffeeExpenses.length} {coffeeExpenses.length === 1 ? 'café' : 'cafés'}
                </p>
              </div>
