import { TrendingUp, Calendar, DollarSign, PieChart, User, Car, Coffee } from 'lucide-react'
...
const ExpenseSummary = ({ expenses, isPersonal = false, householdMembers = [], onFilterPerson = null, categories = [] }) => {
  ...
  // Calcular donaciones de Cafesito App
  const cafesitoDonations = expenses.filter(expense => 
    expense.category === 'donacion' || 
    expense.category === 'cafesito' ||
    expense.description?.toLowerCase().includes('cafesito') ||
    expense.description?.toLowerCase().includes('donación') ||
    expense.description?.toLowerCase().includes('donacion')
  )
  const cafesitoTotal = cafesitoDonations.reduce((sum, expense) => sum + expense.amount, 0)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        {/* Cafesito App - Donaciones */}
        <div className="bg-white p-3 sm:p-4 rounded-lg shadow border hover:shadow-lg hover:scale-105 transition-all duration-200 cursor-pointer col-span-2 sm:col-span-1">
          <div className="space-y-2">
            <p className="text-sm font-medium text-gray-600">Cafesito App</p>
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-lg sm:text-xl font-bold text-gray-900">
                  {formatCurrency(cafesitoTotal)}
                </p>
                <p className="text-sm text-gray-600">
                  {cafesitoDonations.length} {cafesitoDonations.length === 1 ? 'donación' : 'donaciones'}
                </p>
              </div>
              <div className="bg-amber-100 p-2 rounded-lg">
                <Coffee className="h-4 w-4 sm:h-5 sm:w-5 lg:h-8 lg:w-8 text-amber-600" />
              </div>
            </div>
          </div>
        </div>
