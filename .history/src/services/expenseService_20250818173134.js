...
export const returnToHousehold = async (userId) => {
  try {
    ...
    // Marcar al usuario como activo en el hogar nuevamente
    console.log('🔄 Marcando usuario como activo en el hogar...')
    const { error: updateError } = await supabase
      .from('household_members')
    ...
    if (updateError) {
      console.error('❌ Error marcando usuario como activo:', updateError)
      throw updateError
    }
    
    console.log('✅ Usuario marcado como activo en el hogar')
...
export const getPersonalExpenses = async (userId) => {
  try {
    ...
    // Verificar que la tabla expenses existe y es accesible
    console.log('🔍 Verificando acceso a tabla expenses...')
    try {
      ...
      console.log('🔍 Verificación de tabla expenses:', tableCheck)
      console.log('❌ Error verificación tabla:', tableError)
      
      if (tableError) {
        console.error('❌ Tabla expenses no accesible:', tableError)
        return []
      }
    } catch (tableCheckError) {
      console.error('❌ Error verificando tabla expenses:', tableCheckError)
      return []
    }
...
export const getExpenses = async (userId) => {
  try {
    ...
    console.log('🔍 getExpenses - Stack trace:', new Error().stack)
    ...
    // Si está en un hogar, obtener SOLO los gastos del hogar (no los personales)
    console.log('🏠 getExpenses - Usuario está en hogar, obteniendo SOLO gastos del hogar...')
    ...
    // Obtener SOLO los gastos del hogar (con household_id específico)
...
// Agregar un nuevo gasto (personal o del hogar)
export const addExpense = async (expense, userId, isPersonal = false) => {
  try {
    console.log('🔄 addExpense iniciado para expense:', expense)
    console.log('🔄 addExpense - userId:', userId)
    console.log('🔄 addExpense - isPersonal:', isPersonal)
    
    if (isPersonal) {
      // Crear gasto personal (sin hogar)
      console.log('🏠 Usuario en modo personal, creando gasto personal')
      const insertData = {
        description: expense.description,
        amount: expense.amount,
      ...
      // Crear gasto del hogar
      console.log('🏠 Usuario en modo hogar, creando gasto del hogar')
      ...
      if (memberError || !householdMember) {
        throw new Error('Usuario no está en un hogar')
      }
...
export const updateExpense = async (id, expense, userId) => {
  try {
    console.log('🔄 updateExpense iniciado para id:', id)
    console.log('🔄 updateExpense - expense:', expense)
    console.log('🔄 updateExpense - userId:', userId)
...
export const deleteExpense = async (id, userId) => {
  try {
    console.log('🔄 deleteExpense iniciado para id:', id)
    console.log('🔄 deleteExpense - userId:', userId)
...
export const getUserHousehold = async (userId) => {
  try {
    ...
    const { data: memberData, error: memberError } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      // .eq('is_active', true)  // Comentado temporalmente hasta que se ejecute el SQL
      .single()
    ...
    if (memberData) {
      ...
      // Obtener información de los otros miembros del hogar
      try {
        const otherMembers = await getHouseholdMembers(memberData.household_id)
        result.household.members = otherMembers
        console.log('🏠 Miembros del hogar obtenidos:', otherMembers)
      } catch (error) {
        console.error('⚠️ Error obteniendo miembros del hogar:', error)
