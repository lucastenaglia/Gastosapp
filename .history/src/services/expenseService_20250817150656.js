...
// Función alternativa para verificar Aldana sin usar la tabla users
export const checkAldanaHouseholdExpensesSimple = async () => {
  try {
    console.log('🔍 CHECK ALDANA HOUSEHOLD EXPENSES SIMPLE: Iniciando...')
    
    // Usar el ID hardcodeado de Aldana directamente
    const aldanaId = 'a37db74d-5512-429d-ad47-f58d754a483b'
    console.log('🔍 Usando ID de Aldana:', aldanaId)
    
    // 1. Verificar si Aldana está en un hogar
    console.log('🔍 Paso 1: Verificando membresía de Aldana...')
    const { data: aldanaMembership, error: membershipError } = await supabase
      .from('household_members')
      .select('*')
      .eq('user_id', aldanaId)
    ...
    // 2. Obtener todos los gastos del hogar
    console.log('🔍 Paso 2: Obteniendo gastos del hogar...')
    const { data: householdExpenses, error: expensesError } = await supabase
      .from('expenses')
      .select('*')
      .eq('household_id', householdId)
    ...
    // 3. Verificar si hay gastos de Lucas en el hogar
    const lucasId = '2e85b8e1-63ee-4e50-a079-41d98e674f4d'
    const lucasExpenses = householdExpenses.filter(expense => expense.user_id === lucasId)
    ...
    console.error('❌ Error en checkAldanaHouseholdExpensesSimple:', error)
    return { error: 'Error general', details: error }
  }
} 