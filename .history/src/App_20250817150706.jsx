...
import { 
  ...
  checkAldanaStatus,
  checkAldanaHouseholdExpenses,
  checkAldanaHouseholdExpensesSimple
            ...
            <div className="space-x-2">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const result = await verifyAldanaInDatabase()
                    console.log('🔧 Resultado de verificar Aldana:', result)
                  } catch (error) {
                    console.error('❌ Error ejecutando verifyAldanaInDatabase:', error)
                  }
                }}
              ...
              <button
                onClick={async () => {
                  console.log('🔧 Botón debug - Verificar Estado de Aldana...')
                  try {
                    const result = await checkAldanaStatus()
                    console.log('🔧 Estado de Aldana:', result)
                  } catch (error) {
                    console.error('❌ Error ejecutando checkAldanaStatus:', error)
                  }
                }}
                className="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded-lg text-sm"
              >
                🔧 Verificar Estado de Aldana
              </button>
              <button
                onClick={async () => {
                  console.log('🔧 Botón debug - Verificar Gastos de Aldana en Hogar...')
                  try {
                    const result = await checkAldanaHouseholdExpenses(user.id)
                    console.log('🔧 Gastos de Aldana en Hogar:', result)
                  } catch (error) {
                    console.error('❌ Error ejecutando checkAldanaHouseholdExpenses:', error)
                  }
                }}
                className="bg-teal-600 hover:bg-teal-700 text-white px-3 py-2 rounded-lg text-sm"
              >
                🔧 Verificar Gastos de Aldana
              </button>
              <button
                onClick={async () => {
                  console.log('🔧 Botón debug - Verificar Gastos de Aldana (Simple)...')
                  try {
                    const result = await checkAldanaHouseholdExpensesSimple(user.id)
                    console.log('🔧 Gastos de Aldana (Simple):', result)
                  } catch (error) {
                    console.error('❌ Error ejecutando checkAldanaHouseholdExpensesSimple:', error)
                  }
                }}
                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-sm"
              >
                🔧 Verificar Gastos de Aldana (Simple)
              </button>
            </div>
          </div>
        </div>
